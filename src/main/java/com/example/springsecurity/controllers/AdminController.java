...
import com.example.springsecurity.enumm.Role;
...
import com.example.springsecurity.services.ImageService;
...
public class AdminController {
    ...
    public String admin(Model model) {
        // при переходе по ссылке /admin/product передаем также все продукты, которые есть в репозитории
    ...
    public String addProduct(@ModelAttribute("product") @Valid Product product, BindingResult bindingResult,
        ...
        // Если ошибок нет, то Обрабатываем загружаемые файлы с файл Инпутов формы.
        // Если файл не пустой
        if (file_one != null) {
    ...
    public String editProduct(Model model, @PathVariable("id") int id) {
        // Кладём в модель продукт по id и категории товаров. Благодаря этому заполним все поля
